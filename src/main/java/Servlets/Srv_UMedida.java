 ...
 */
@WebServlet("/Srv_UMedida")
public class Srv_UMedida extends HttpServlet implements Servlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //--------------------------INICIA----
        System.out.println("--------------------Ingresa a Srv_UMedida---------------------");
        String act = request.getParameter("action"); //Nombre del grupo de botones o inputs
        obj_Mensaje mensaje = new obj_Mensaje();

        obj_UMedida um = null;
        dao_UMedida call = null;
        obj_Usuario usuario = null;
        Validaciones validar = new Validaciones();
        boolean rsp = false;
        //Mostrar lista
        dao_UMedida dao3 = null;
        List<obj_UMedida> umlista = null;

        //TOMA LOS VALORES DEL FORMULARIO 
        String id = request.getParameter("idUnidadMedida");
        String clave = request.getParameter("claveUnidadMedida");
        String desc = request.getParameter("descripcionUnidadMedida");

        //Muestra lo que toma de formulario
        System.out.println("---------------------Srv_UMedida---------------------");
        ...
        if (act == null) {
            ...
            System.out.println("Srv_UMedida- Ningun boton- Ingresa a TRY");
            try {
                System.out.println("Srv_UMedida- Lista servlet- Ingresa a TRY");
                dao3 = new dao_UMedida();
                umlista = dao3.Listar();
            ...
            request.getRequestDispatcher("CatalogosSat/UnidadMedida.jsp").forward(request, response);
            return;

            //--------------------------------------------GUARDAR---------------------------------------------------------------  
        } else {
            try {
                call = new dao_UMedida();
                um = new obj_UMedida();

                um.setClaveUnidadMedida(clave);
                um.setDescripcionUnidadMedida(desc);
                um.setUsuario(usuario.getUsuario());//Nombre usuario

                mensaje = call.ctg_ud_medida(act, um, id);
                session.setAttribute("mensaje", mensaje);

                dao3 = new dao_UMedida();
                umlista = dao3.Listar();
                session.setAttribute("umlista", umlista);

                response.sendRedirect("Srv_UMedida");
                return;
