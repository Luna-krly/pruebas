 ...
 */
@WebServlet("/Srv_TCompr")
public class Srv_TCompr extends HttpServlet implements Servlet {
...
//--------------------------INICIA----
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("--------------------Ingresa a Srv_TCompr---------------------");
        ...
        dao_TComprobante call = null;
        obj_Usuario usuario = null;
        Validaciones validar = new Validaciones();
        boolean rsp = false;
        //Mostrar lista
        dao_TComprobante dao3 = null;
        List<obj_TComprobante> tclista = null;

        //TOMA LOS VALORES DEL FORMULARIO 
        String id = request.getParameter("idTipoComprobante");
        String clave = request.getParameter("claveTipoComprobante");
        String desc = request.getParameter("descripcionComprobante");

        //Muestra lo que toma de formulario
        System.out.println("---------------------Srv_TCompr---------------------");
        ...
        if (act == null) {
            ...
            System.out.println("Srv_TCompr- Ningun boton- Ingresa a TRY");
            try {
                System.out.println("Srv_TCompr- Lista servlet- Ingresa a TRY");
                dao3 = new dao_TComprobante();
                tclista = dao3.Listar();
                request.setAttribute("tclista", tclista);
                System.out.println("Srv_TCompr- Lista servlet- Envia lista");
            ...
            request.getRequestDispatcher("CatalogosSat/Comprobantes.jsp").forward(request, response);
            return;

            //--------------------------------------------GUARDAR---------------------------------------------------------------  
        } else {
            try {
                call = new dao_TComprobante();
                tcompr = new obj_TComprobante();

                tcompr.setClaveTipoComprobante(clave);
                tcompr.setDescripcionTipoComprobante(desc);
                tcompr.setUsuario(usuario.getUsuario());//Nombre usuario

                mensaje = call.ctg_tipo_comprob(act, tcompr, id);
                session.setAttribute("mensaje", mensaje);

                dao3 = new dao_TComprobante();
                tclista = dao3.Listar();
                session.setAttribute("tclista", tclista);

                response.sendRedirect("Srv_TCompr");
                return;
