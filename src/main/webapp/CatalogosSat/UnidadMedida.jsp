<%-- 
    Document   : u_medida
    Created on : 1 mar 2024, 17:05:16
    Author     : Lezly Oliván
--%>
...
<%
    System.out.println("---------------------UNIDAD DE MEDIDA JSP---------------------");
%>
<style>
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE UNIDAD DE MEDIDA</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form action="Srv_UMedida" method="POST" id="formulario" class="fontformulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text" class="form-control form-control-sm" id="idUnidadMedida" name="idUnidadMedida" value="" hidden="true"/>
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text"  id="claveUnidadMedida" name="claveUnidadMedida" value="" class="form-control" maxlength="3"  pattern="^[A-Za-z0-9]{2,3}$" title="La clave solo puede constar de numeros o letras con una longitud de 2 a 3 carcteres" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="claveUnidadMedida">CLAVE</label>
                </div>
            </div>
            <div class="col">
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" id="descripcionUnidadMedida" name="descripcionUnidadMedida" value="" class="form-control" pattern="^(?!\s)[^\s]+(?:\s[^\s]+)*(?<!\s)$" title="El campo debe contener solo letras y no espacios al principio o al final." maxlength="200" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="descripcionUnidadMedida">DESCRIPCIÓN UNIDAD DE MEDIDA</label>
                </div>
            </div>
        </div>
        <br>
    ...
    <div class="row d-flex justify-content-center p-3 stc">
        ...
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="220" title="IMPRIMIR EN PDF" class="btn btn-sm btn-rounded"style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span> </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="EXPORTAR A EXCEL" class="btn btn-outline-success btn-sm btn-rounded" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('unitOfMeasure-table', 'UnidadMedida.xls')" hidden="true">
    ...
    <div class="row d-flex justify-content-center p-3">
        <div class="text-center" style="position: relative; width: 80%;">
            <div class="table-responsive" style="height: 270px;">
                <c:if test="${not empty umlista}">
                    <table id="unitOfMeasure-table" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                <th class="d-none">*</th>
                                <th class="fw-bold">CLAVE</th>
                                <th class="fw-bold">DESCRIPCIÓN UNIDAD DE MEDIDA</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="unidadDeMedida" items="${umlista}">
                                <tr>
                                    <td class="text-center id-unidadmedida d-none">${unidadDeMedida.idUnidadMedida}</td>
                                    <td class="texto-largo">${unidadDeMedida.claveUnidadMedida}</td>
                                    <td class="texto-largo">${unidadDeMedida.descripcionUnidadMedida}</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty umlista}">
                    <center>
                        <BR>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let idUnidadMedida = fila.querySelector('.id-unidadmedida').innerText;
                let claveUnidadMedida = fila.querySelector('.texto-largo:nth-child(2)').innerText;
                let descripcionUnidadMedida = fila.querySelector('.texto-largo:nth-child(3)').innerText;


                // Ingresa los valores a los cuadros de texto
                formulario.querySelector("#idUnidadMedida").value = idUnidadMedida;
                formulario.querySelector("#claveUnidadMedida").value = claveUnidadMedida;
                formulario.querySelector("#descripcionUnidadMedida").value = descripcionUnidadMedida;
                formulario.querySelector("#claveUnidadMedida").readOnly = true;



                formulario.querySelector('#idUnidadMedida').focus();
                formulario.querySelector('#claveUnidadMedida').focus();
                formulario.querySelector('#descripcionUnidadMedida').focus();
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.getElementById("claveUnidadMedida").blur();
            document.getElementById("descripcionUnidadMedida").blur();
            document.querySelector("#claveUnidadMedida").readOnly = false;
            document.querySelector("#idUnidadMedida").readOnly = false;
            document.querySelector("#descripcionUnidadMedida").readOnly = false;
