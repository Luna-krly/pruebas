<%-- 
    Document   : t_factor
    Created on : 1 mar 2024, 16:58:20
    Author     : Lezly Oliván
--%>
...
<%
    System.out.println("---------------------TIPO DE FACTOR JSP---------------------");
%>
<style>
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE TIPO DE FACTOR</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form id="formulario"  action="Srv_TFactor" method="POST" class="fontformulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text"  id="idFactor" name="idFactor" class="form-control" value="" hidden="true"/>
                <div data-mdb-input-init class="form-outline" style="background-color: white;">                    
                    <input style="font-weight: bold;" type="text" id="claveFactor" name="claveFactor" class="form-control" maxlength="6"   pattern="^[A-Za-z]{0,6}$" title="La clave debe seguir debe constar de unicamente letras." onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="claveFactor">CLAVE TIPO DE FACTOR</label>
                </div>
            </div>
        </div>
        <br>
    ...
    <div class="row d-flex justify-content-center p-3 ">
        ...
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="217" title="Imprimir" class="stc btn btn-primary btn-sm btn-rounded" style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span> </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="Imprimir" class="stc btn btn-outline-success btn-sm btn-rounded" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('tablaFactores', 'TipoFactor.xls')" hidden="true">
    ...
    <div class="row d-flex justify-content-center p-3">
        <div class="text-center" style="position: relative; width: 80%;">
            <div class="table-responsive" style="height: 270px;">
                <c:if test="${not empty tflista}">
                    <table id="tablaFactores" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                <th class="d-none">#</th>
                                <th class="fw-bold">CLAVE TIPO DE FACTOR</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="tipoFactor" items="${tflista}">
                                <tr>
                                    <td class="text-center id-tipofactor d-none">${tipoFactor.idTipoFactor}</td>
                                    <td class="texto-largo">${tipoFactor.claveTipoFactor}</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty tflista}">
                    <center>
                        <BR>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let idFactor = fila.querySelector('.id-tipofactor').innerText;
                let claveFactor = fila.querySelector('.texto-largo:nth-child(2)').innerText;

                // Ingresa los valores a los cuadros de texto
                formulario.querySelector("#idFactor").value = idFactor;
                formulario.querySelector("#claveFactor").value = claveFactor;
                // formulario.querySelector("#claveFactor").readOnly = true;



                formulario.querySelector('#idFactor').focus();
                formulario.querySelector('#claveFactor').focus();
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.getElementById("idFactor").blur();
            document.getElementById("claveFactor").blur();
            // document.querySelector("#claveFactor").readOnly = false;
