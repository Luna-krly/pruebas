<%-- 
    Document   : monedas
    Created on : 1 mar 2024, 17:20:21
    Author     : Lezly Oliván
--%>
...
<%
    System.out.println("---------------------Monedas JSP---------------------");
%>
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE MONEDAS</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form action="Srv_Monedas" method="POST" class="fontformulario" id="formulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text" class="form-control form-control-sm" id="idMoneda" name="idMoneda" value="" hidden/>
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <!--------->
                    <input style="font-weight: bold;" type="text" class="form-control" id="claveMoneda" name="claveMoneda" value="" maxlength="3" pattern="^[A-Za-z]{3}$" title="La clave debe seguir el formato de tres letras, por ejemplo AAA" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="claveMoneda">(3)  CLAVE</label>
                </div>
            </div>
            <div class="col">
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" class="form-control" id="descripcionMoneda" name="descripcionMoneda" pattern="^(?!\s)[^\s]+(?:\s[^\s]+)*(?<!\s)$" title="El campo debe contener solo letras y no espacios al principio o al final." maxlength="60" value="" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="descripcionMoneda">DESCRIPCIÓN MONEDA</label>
                </div>
            </div>
        </div>
        <br>
    ...
    <div class="row d-flex justify-content-center p-3 ">
        ...
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="211" title="IMPRIMIR EN PDF" class="stc btn btn-primary btn-sm btn-rounded" style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span> </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="EXPORTAR A EXCEL" class="btn btn-outline-success btn-sm btn-rounded stc" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('obejctTaxes-table', 'Monedas.xls')" hidden="true">
    ...
    <div class="row d-flex justify-content-center p-3">
        <!-- Añadir este div entre los dos -->
        <div class="text-center" style="position: relative; width: 80%;">
            <div class="table-responsive" style="height: 270px;">
                <c:if test="${not empty monedalista}">
                    <table id="obejctTaxes-table" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                <th class="d-none">*</th>
                                <th class="fw-bold">CLAVE</th>
                                <th class="fw-bold">DESCRIPCIÓN DE MONEDA</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="moneda" items="${monedalista}">
                                <tr>
                                    <td class="text-center id-moneda d-none">${moneda.idMoneda}</td>
                                    <td class="texto-largo">${moneda.claveMoneda}</td>
                                    <td class="texto-largo">${moneda.descripcionMoneda}</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty monedalista}">
                    <center>
                        <br>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let id = fila.querySelector('.id-moneda').innerText;
                let claveMoneda = fila.querySelector('.texto-largo:nth-child(2)').innerText;
                let descripcionMoneda = fila.querySelector('.texto-largo:nth-child(3)').innerText;




                // Ingresa los valores a los cuadros de texto
                formulario.querySelector("#idMoneda").value = id;
                formulario.querySelector("#claveMoneda").value = claveMoneda;
                formulario.querySelector("#descripcionMoneda").value = descripcionMoneda;
                formulario.querySelector("#claveMoneda").readOnly = true;



                formulario.querySelector("#idMoneda").focus();
                formulario.querySelector('#claveMoneda').focus();
                formulario.querySelector('#descripcionMoneda').focus();
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.getElementById("idMoneda").blur();
            document.getElementById("claveMoneda").blur();
            document.getElementById("descripcionMoneda").blur();
            document.querySelector("#claveMoneda").readOnly = false;
            document.querySelector("#idMoneda").readOnly = false;
            document.querySelector("#descripcionMoneda").readOnly = false;
