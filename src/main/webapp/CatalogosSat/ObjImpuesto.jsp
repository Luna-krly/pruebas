<%-- 
    Document   : o_impuesto
    Created on : 1 mar 2024, 17:31:59
    Author     : Lezly Oliván
--%>
...
<%
    System.out.println("---------------------OBJETO DE IMPUESTO JSP---------------------");
%>
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE OBJETO DE IMPUESTO</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form action="Srv_OImpuesto" method="POST" id="formulario" class="fontformulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text" class="form-control form-control-sm" id="idObjetoImpuesto" name="idObjetoImpuesto" value="" hidden/>
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" id="claveObjetoImpuesto" name="claveObjetoImpuesto" value="" class="form-control" maxlength="2" pattern="^[0-9]{2}$" title="La clave debe seguir el formato de dos dígitos que van del 0 al 9"pattern="^[A-Za-z]{3}$" title="La clave debe seguir el formato de tres letras, por ejemplo MNX" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="claveObjetoImpuesto">(2)  CLAVE</label>
                </div>
            </div>
            <div class="col">
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" id="descripcionObjetoImpuesto" name="descripcionObjetoImpuesto" value="" class="form-control" pattern="^(?!\s)[^\s]+(?:\s[^\s]+)*(?<!\s)$" title="El campo debe contener solo letras y no espacios al principio o al final." maxlength="60" onblur="this.value = this.value.trim()" required=""/>
                    <label class="form-label" for="descripcionObjetoImpuesto">DESCRIPCIÓN OBJETO DE IMPUESTO</label>
                </div>
            </div>
        </div>
        <br>
    ...
    <div class="row d-flex justify-content-center p-3 ">
        ...
        <div class="col-md-3" style="text-align: center;">
            <div data-mdb-input-init class="form-outline fontformulario">
                ...
                <label class="form-label fontformulario" for="busqueda">BUSCAR</label>
            </div>
        </div>
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="212" title="IMPRIMIR EN PDF" class="stc btn btn-primary btn-sm btn-rounded" style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span> </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="EXPORTAR A EXCEL" class="btn btn-outline-success btn-sm btn-rounded stc" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('payType-table', 'ObjImpuesto.xls')" hidden="true">
                    ...
                    <table id="payType-table" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                ...
                                <th class="fw-bold">DESCRIPCIÓN DE OBJETO DE IMPUESTO</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="objetoDeImpuesto" items="${oimplista}">
                                <tr>
                                    <td class="text-center id-objetoimpuesto d-none">${objetoDeImpuesto.idObjetoImpuesto}</td>
                                    <td class="texto-largo">${objetoDeImpuesto.clave}</td>
                                    <td class="texto-largo">${objetoDeImpuesto.descripcion}</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty oimplista}">
                    <center>
                        <br>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let idObjetoImpuesto = fila.querySelector('.id-objetoimpuesto').innerText;
                let claveObjetoImpuesto = fila.querySelector('.texto-largo:nth-child(2)').innerText;
                let descripcionObjetoImpuesto = fila.querySelector('.texto-largo:nth-child(3)').innerText;


                // Ingresa los valores a los cuadros de texto
                formulario.querySelector("#idObjetoImpuesto").value = idObjetoImpuesto;
                formulario.querySelector("#claveObjetoImpuesto").value = claveObjetoImpuesto;
                formulario.querySelector("#descripcionObjetoImpuesto").value = descripcionObjetoImpuesto;
                formulario.querySelector("#claveObjetoImpuesto").readOnly = true;




                formulario.querySelector('#claveObjetoImpuesto').focus();
                formulario.querySelector('#descripcionObjetoImpuesto').focus();
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.getElementById("claveObjetoImpuesto").blur();
            document.getElementById("descripcionObjetoImpuesto").blur();
            document.querySelector("#idObjetoImpuesto").readOnly = false;
            document.querySelector("#descripcionObjetoImpuesto").readOnly = false;
            document.querySelector("#claveObjetoImpuesto").readOnly = false;
