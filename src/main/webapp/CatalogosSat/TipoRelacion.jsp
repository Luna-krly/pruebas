<%-- 
    Document   : t_relacion
    Created on : 1 mar 2024, 17:01:14
    Author     : Lezly Oliván
--%>
...
<%
    System.out.println("---------------------TIPO DE RELACIÓN JSP---------------------");
%>
<style>
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE TIPO DE RELACIÓN</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form id="formulario"  action="Srv_TRelacion" method="POST" class="fontformulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text"  id="idRelacion" name="idRelacion" class="form-control" value="" hidden="true"/>
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" id="claveRelacion" name="claveRelacion" class="form-control " maxlength="2" required  pattern="^[0-9]{2}$" title="La clave debe estar formada por dos digitos, por ejemplo 01" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="claveRelacion">(2)  CLAVE</label>
                </div>
            </div>
            <div class="col">
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" id="descripcionRelacion" name="descripcionRelacion" class="form-control " pattern="^(?!\s)[^\s]+(?:\s[^\s]+)*(?<!\s)$" title="El campo debe contener solo letras y no espacios al principio o al final." maxlength="150" onblur="this.value = this.value.trim()" required="true"/>
                    <label class="form-label" for="descripcionRelacion">DESCRIPCIÓN TIPO DE RELACIÓN</label>
                </div>
            </div>
        </div>
        <br>
    ...
    <div class="row d-flex justify-content-center p-3 stc">
        ...
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="218" title="Imprimir" class="btn btn-primary btn-sm btn-rounded" style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span> </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="Imprimir" class="btn btn-outline-success btn-sm btn-rounded" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('tablaRelaciones', 'TipoRelacion.xls')" hidden="true">
    ...
    <div class="row d-flex justify-content-center p-3">
        <div class="text-center" style="position: relative; width: 80%;">    
            <div class="table-responsive" style="height: 270px;">
                <c:if test="${not empty trlista}">
                    <table id="tablaRelaciones" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                <th class="d-none">#</th>
                                <th class="fw-bold">CLAVE</th>
                                <th class="fw-bold">DESCRIPCIÓN TIPO DE RELACIÓN</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="tipoRelacion" items="${trlista}">
                                <tr>
                                    <td class="text-center id-tiporelacion d-none">${tipoRelacion.idTipoRelacion}</td>
                                    <td class="texto-largo">${tipoRelacion.claveTipoRelacion}</td>
                                    <td class="texto-largo">${tipoRelacion.descripcionTipoRelacion}</td>  
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty trlista}">
                    <center>
                        <BR>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let idRelacion = fila.querySelector('.id-tiporelacion').innerText;
                let claveRelacion = fila.querySelector('.texto-largo:nth-child(2)').innerText;
                let descripcionRelacion = fila.querySelector('.texto-largo:nth-child(3)').innerText;


                // Ingresa los valores a los cuadros de texto
                formulario.querySelector("#idRelacion").value = idRelacion;
                formulario.querySelector("#claveRelacion").value = claveRelacion;
                formulario.querySelector("#descripcionRelacion").value = descripcionRelacion;
                formulario.querySelector("#claveRelacion").readOnly = true;



                formulario.querySelector('#idRelacion').focus();
                formulario.querySelector('#claveRelacion').focus();
                formulario.querySelector('#descripcionRelacion').focus();
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.getElementById("idRelacion").blur();
            document.getElementById("claveRelacion").blur();
            document.getElementById("descripcionRelacion").blur();
            document.querySelector("#claveRelacion").readOnly = false;
            document.querySelector("#idRelacion").readOnly = false;
            document.querySelector("#descripcionRelacion").readOnly = false;
