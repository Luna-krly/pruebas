<%-- 
    Document   : w_paises
    Created on : 13/03/2023, 12:33:32 PM
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE PAÍSES</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form action="Srv_Paises" method="POST" id="formulario" class="fontformulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text" class="form-control form-control-sm" id="idPais" name="idPais" value="" hidden="false"/>
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" id="clavePais" name="clavePais" value="" class="form-control" maxlength="3" pattern="^[A-Za-z]{3}$" title="La clave debe seguir el formato de tres letras, por ejemplo USA" onblur="this.value = this.value.trim()" required />
                    <label class="form-label"  for="cvePais">(3) CLAVE</label>
                </div>
            </div>
            <div class="col">
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" class="form-control" id="descripcionPais" name="descripcionPais" value="" pattern="^(?!\s)[^\s]+(?:\s[^\s]+)*(?<!\s)$" title="El campo debe contener solo letras y no espacios al principio o al final." maxlength="60" pattern="[0-9[A-Za-zÁ-Úá-ú\s]+" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="dscP1">DESCRIPCIÓN PAÍS</label>
                </div>
            </div>
        </div>
        <br>
    ...
    <div class="row d-flex justify-content-center p-3">
        ...
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="213" title="Imprimir" class="stc btn btn-primary btn-sm btn-rounded" style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span> </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="Imprimir" class="btn btn-outline-success btn-sm btn-rounded stc" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('contry-table', 'Paises.xls')" hidden="true">
    ...
    <div class="row d-flex justify-content-center p-3">
        <div class="text-center" style="position: relative; width: 80%;">
            <div class="table-responsive" style="height: 270px;">
                <c:if test="${not empty paislista}">
                    <table id="contry-table" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                <th class="d-none">*</th>
                                <th class="fw-bold">CLAVE</th>
                                <th class="fw-bold">DESCRIPCIÓN PAÍS</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="pais" items="${paislista}">
                                <tr>
                                    <td class="text-center id-pais d-none">${pais.idPais}</td>
                                    <td class="texto-largo">${pais.clavePais}</td>
                                    <td class="texto-largo">${pais.descripcionPais}</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty paislista}">
                    <center>
                        <br>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let idPais = fila.querySelector('.id-pais').innerText;
                let clavePais = fila.querySelector('.texto-largo:nth-child(2)').innerText;
                let descripcioPais = fila.querySelector('.texto-largo:nth-child(3)').innerText;

                // Ingresa los valores a los cuadros de texto
                formulario.querySelector("#idPais").value = idPais;
                formulario.querySelector("#clavePais").value = clavePais;
                formulario.querySelector("#descripcionPais").value = descripcioPais;
                formulario.querySelector("#clavePais").readOnly = true;



                formulario.querySelector('#idPais').focus();
                formulario.querySelector('#clavePais').focus();
                formulario.querySelector('#descripcionPais').focus();
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.getElementById("clavePais").blur();
            document.getElementById("descripcionPais").blur();
            document.getElementById("idPais").blur();
            document.querySelector("#clavePais").readOnly = false;
            formulario.querySelector("#idPais").readOnly = false;
            formulario.querySelector("#descripcionPais").readOnly = false;
