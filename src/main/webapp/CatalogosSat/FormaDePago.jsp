<%-- 
    Document   : f_pago
    Created on : 1 mar 2024, 16:45:07
    Author     : Lezly Oliván
--%>
...
<%
    System.out.println("---------------------FORMA DE PAGO JSP---------------------");
%>
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE FORMA DE PAGO</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form  action="Srv_FormaPago" method="POST" id="formulario" class="fontformulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text" class="form-control form-control-sm" id="idFormaDePago" name="idFormaDePago" value="" hidden/>
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text"  id="claveFormaPago" name="claveFormaPago" value="" class="form-control " maxlength="2" pattern="^[0-9]{2}$" title="La clave debe seguir el formato de dos dígitos que van del 0 al 9" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="claveFormaPago">(2)  CLAVE</label>
                </div>
            </div>
            <div class="col">
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text"id="descripcionFormaPago" name="descripcionFormaPago" value="" class="form-control" pattern="^(?!\s)[^\s]+(?:\s[^\s]+)*(?<!\s)$" title="El campo debe contener solo letras y no espacios al principio o al final." maxlength="60" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="descripcionFormaPago">DESCRIPCIÓN FORMA DE PAGO</label>
                </div>
            </div>
        </div>
        <br>
    ...
    <div class="row d-flex justify-content-center p-3 ">
        ...
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="208" title="IMPRIMIR EN PDF" class="stc btn btn-primary btn-sm btn-rounded" style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span>
                    </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="EXPORTAR A EXCEL" class="btn btn-outline-success btn-sm btn-rounded stc" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('payType-table', 'FormaDePago.xls')" hidden="true">
    ...
    <div class="row d-flex justify-content-center p-3">
        <div class="text-center" style="position: relative; width: 80%;">
            <div class="table-responsive" style="height: 270px;">
                <c:if test="${not empty formaPagolista}">
                    <table id="payType-table" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                ...
                                <th class="fw-bold">DESCRIPCIÓN FORMA DE PAGO</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="formaPago" items="${formaPagolista}">
                                <tr>
                                    <td class="text-center id-formapago d-none">${formaPago.idFormaPago}</td>
                                    <td class="texto-largo">${formaPago.claveFormaPago}</td>
                                    <td class="texto-largo">${formaPago.descripcionFormaPago}</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty formaPagolista}">
                    <center>
                        <br>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let id = fila.querySelector('.id-formapago').innerText;
                ...
                formulario.querySelector("#idFormaDePago").value = id;
                formulario.querySelector("#claveFormaPago").value = claveMoneda;
                formulario.querySelector("#descripcionFormaPago").value = descripcionMoneda;


                formulario.querySelector('#idFormaDePago').focus();
                formulario.querySelector('#claveFormaPago').focus();
                formulario.querySelector('#descripcionFormaPago').focus();
                formulario.querySelector("#claveFormaPago").readOnly = true;
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.getElementById("idFormaDePago").blur();
            document.getElementById("claveFormaPago").blur();
            document.getElementById("descripcionFormaPago").blur();
            formulario.querySelector("#claveFormaPago").readOnly = false;
            formulario.querySelector("#descripcionFormaPago").readOnly = false;
            formulario.querySelector("#idFormaDePago").readOnly = false;
