<%-- 
    Document   : t_ingresos
    Created on : 1 mar 2024, 13:29:52
    Author     : Lezly Oliván
--%>
...
<%
    System.out.println("---------------------TIPO DE SERVICIO JSP---------------------");
%>
<style>
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE TIPO DE SERVICIOS</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form id="formulario"  action="Srv_TServicio" method="POST" class="fontformulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text"  id="idServicio" name="idServicio" class="form-control" value="" hidden="true"/>
                <div data-mdb-input-init class="form-outline" style="background-color: white;">                    
                    <input style="font-weight: bold;" type="text" id="claveServicio" name="claveServicio" class="form-control" maxlength="3" required="true"  pattern="^[0-9]{3}$" title="La clave debe estar formada por dos digitos, por ejemplo 001." onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="claveServicio">(3)  CLAVE</label>
                </div>
            </div>
            <div class="col">
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" id="descripcionServicio" name="descripcionServicio" class="form-control" pattern="^(?!\s)[^\s]+(?:\s[^\s]+)*(?<!\s)$" title="El campo debe contener solo letras y no espacios al principio o al final." maxlength="25" onblur="this.value = this.value.trim()" required="true"/>
                    <label class="form-label" for="descripcionServicio">DESCRIPCIÓN DE TIPO DE SERVICIO</label>
                </div>
            </div>
        </div>
        <br>
    ...
    <div class="row d-flex justify-content-center p-3 stc">
        ...
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="219" title="IMPRIMIR EN PDF" class="btn btn-sm btn-rounded" style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span> </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="EXPORTAR A EXCEL" class="btn btn-outline-success btn-sm btn-rounded" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('tablaIngresos', 'TipoServicio.xls')" hidden="true">
    ...
    <div class="row d-flex justify-content-center p-3">
        <div class="text-center" style="position: relative; width: 80%;">    
            <div class="table-responsive" style="height: 270px;">
                <c:if test="${not empty srvlista}">
                    <table id="tablaIngresos" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                <th class="d-none">#</th>
                                <th class="fw-bold">CLAVE</th>
                                <th class="fw-bold">DESCRIPCIÓN TIPO DE SERVICIO</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="servicio" items="${srvlista}">
                                <tr>
                                    <td class="text-center id-tiposervicio d-none">${servicio.idTServicio}</td>
                                    <td class="texto-largo">${servicio.clave}</td>
                                    <td class="texto-largo">${servicio.descripcion}</td> 
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty srvlista}">
                    <center>
                        <BR>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let idServicio = fila.querySelector('.id-tiposervicio').innerText;
                let claveServicio = fila.querySelector('.texto-largo:nth-child(2)').innerText;
                let descripcionServicio = fila.querySelector('.texto-largo:nth-child(3)').innerText;


                // Ingresa los valores a los cuadros de texto
                formulario.querySelector("#idServicio").value = idServicio;
                formulario.querySelector("#claveServicio").value = claveServicio;
                formulario.querySelector("#descripcionServicio").value = descripcionServicio;
                formulario.querySelector("#claveServicio").readOnly = true;



                formulario.querySelector('#idServicio').focus();
                formulario.querySelector('#claveServicio').focus();
                formulario.querySelector('#descripcionServicio').focus();
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.getElementById("idServicio").blur();
            document.getElementById("claveServicio").blur();
            document.getElementById("descripcionServicio").blur();
            document.querySelector("#claveServicio").readOnly = false;
            document.querySelector("#idServicio").readOnly = false;
            document.querySelector("#descripcionServicio").readOnly = false;
