<%-- 
    Document   : cfdi
    Created on : 1 mar 2024, 17:17:02
    Author     : Lezly Oliván
--%>
...
<%
    System.out.println("---------------------USO DE CFDI JSP---------------------");
%>
<style>
...
<main class="col ps-md-2 pt-2 mx-3">
    ...
    <div class="page-header pt-3 stc">
        <h3>CATÁLOGO DE USO DE CFDI</h3>
    </div>
    <div id="datetime" class="stc" style="font-size: 75%;">
    </div>
    <hr>
    <form action="Srv_UCFDI" method="POST" class="fontformulario" id="formulario" style="font-size: 80%; margin: 15px;">
        <!-- 2 column grid layout with text inputs for the first and last names -->
        <div class="row mb-4">
            <div class="col">
                <input type="text" class="form-control form-control-sm"  id="idCFDI" name="idCFDI" value="" hidden/> 
                <div data-mdb-input-init class="form-outline" style="background-color: white;">                   
                    <input style="font-weight: bold;" type="text" id="claveUsoCFDI" name="claveUsoCFDI" class="form-control " maxlength="4"  pattern="^[A-Za-z0-9]{3,4}$" title="La clave solo puede constar de numeros o letras con una longitud de 3 a 4 carcteres" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="claveUsoCFI">CLAVE</label>
                </div>
            </div>
            <div class="col">
                <div data-mdb-input-init class="form-outline" style="background-color: white;">
                    <input style="font-weight: bold;" type="text" id="descripcionUsoCFDI"  name="descripcionUsoCFDI" class="form-control " pattern="^(?!\s)[^\s]+(?:\s[^\s]+)*(?<!\s)$" title="El campo debe contener solo letras y no espacios al principio o al final." maxlength="200" onblur="this.value = this.value.trim()" required/>
                    <label class="form-label" for="descripcionUsoCFDI">DESCRIPCIÓN USO DE CFDI</label>
                </div>
            </div>
        </div>
        <style>
    ...
    <div class="row d-flex justify-content-center p-3 stc">
        ...
        <div class="col-md-4" style="text-align: left;">
            ...
            <form action="${pageContext.request.contextPath}/Reportes/reportes_catalogos.jsp" id="formulario-impresion" method="post" target="_blank">
                <th><button type="submit" name="action" value="221" title="IMPRIMIR EN PDF" class="btn btn-sm btn-rounded" style="color:white; background-color: purple;" data-mdb-ripple-init>
                        <i class="fas fa-print"></i>
                        <span class="d-none d-sm-inline">IMPRIMIR EN PDF</span> </button></th>
            </form>
            <button type="button" id="imprimir-btn" title="EXPORTAR A EXCEL" class="btn btn-outline-success btn-sm btn-rounded" data-mdb-ripple-init  data-mdb-ripple-color="dark" onclick="exportTableToExcel('tableCFI', 'CFDI.xls')" hidden="true">
    ...
    <div class="row d-flex justify-content-center p-3">
        <div class="text-center" style="position: relative; width: 80%;">
            <div class="table-responsive" style="height: 270px;">
                <c:if test="${not empty uclista}">
                    <table id="tableCFI" class="fontformulario table table-hover fonthead table-bordered table-sm" style="font-size: 90%; background-color: #fff;">
                        <thead class="table-secondary">
                            <tr>
                                <th class="d-none">*</th>
                                <th class="fw-bold">CLAVE</th>
                                <th class="fw-bold">DESCRIPCIÓN DE USO CFDI</th>
                                <th class="fw-bold">FISICA</th>
                                <th class="fw-bold">MORAL</th>
                            </tr>
                        </thead>
                        <tbody id="datos" style="font-size: 80%;">
                            <c:forEach var="CFDI" items="${uclista}">
                                <tr>
                                    <td class="text-center id-usocfdi d-none">${CFDI.idUsoCFDI}</td>
                                    <td class="texto-largo">${CFDI.claveUsoCFDI}</td>
                                    <td class="texto-largo">${CFDI.descripcionUsoCFDI}</td>
                                    <td class="texto-largo">${CFDI.fisica}</td>
                                    <td class="texto-largo">${CFDI.moral}</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </c:if>
                <c:if test="${empty uclista}">
                    <center>
                        <BR>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabla.addEventListener('click', function (event) {
            ...
            if (event.target.tagName === 'TD') {
                ...
                let idCFDI = fila.querySelector('.id-usocfdi').innerText;
                let claveUsoCFDI = fila.querySelector('.texto-largo:nth-child(2)').innerText;
                let descripcionUsoCFDI = fila.querySelector('.texto-largo:nth-child(3)').innerText;
...
//              Ingresa los valores a los cuadros de texto
                formulario.querySelector("#idCFDI").value = idCFDI;
                formulario.querySelector("#claveUsoCFDI").value = claveUsoCFDI;
                formulario.querySelector("#descripcionUsoCFDI").value = descripcionUsoCFDI;
                ...
                formulario.querySelector("#claveUsoCFDI").readOnly = true;




                formulario.querySelector('#idCFDI').focus();
                formulario.querySelector('#claveUsoCFDI').focus();
                formulario.querySelector('#descripcionUsoCFDI').focus();
            }
        });
    });
...
//*****************Funcion limpiar para quitar boton y estiñps en tabla 
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#Limpiar').addEventListener('click', function () {
            document.querySelector("#claveUsoCFDI").readOnly = false;
            document.querySelector("#idCFDI").readOnly = true;
            document.querySelector("#esFisica").disabled = false;
            document.querySelector("#esMoral").disabled = false;
            document.querySelector("#descripcionUsoCFDI").readOnly = false;
            document.getElementById('spanNoMoral').hidden = false;
            document.getElementById('spanSiMoral').hidden = true;
            document.getElementById('spanNoFisica').hidden = false;
            document.getElementById('spanSiFisica').hidden = true;
